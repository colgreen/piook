...
Each transmission consists of a series of on-off transitions of the transmitter that are observed on the data
...
Notes.
   ...
   * The first three nibbles can be considered to be a fixed sequence/preamble that can be used to detect the start of a message; however, the 
   first nibble tends to be masked with noise, hence piook tests for the sequence 0xF8 instead of 0xFF8. I imagine additional leading bits are 
   present to allow for just such lead-in-noise issues.

   * Nibbles 3 and 4 straddle a byte boundary and contain a unique code that is generated by the remote unit when it is powered on. I haven't
   examined how this code changes to observe if there is any pattern, but I it's possible that it's randomly generated. The purpose of this code
